...
import { MoleculerAction, MoleculerService, MoleculerServiceStarted } from "moleculer-plus";
...
export default class UsersService extends Service {
    ...
    public async FindUserByLogin(ctx: Context<FindUserByLoginDto>): Promise<User | null> {
    ...
    public async RegisterUser(ctx: Context<RegisterUserDto>): Promise<User | null> {
    ...
    public async LoginSessionUser(ctx: Context<LoginSessionUserDto>): Promise<Session | null> {
        const { UserIdentifier } = ctx.params;

        const user = await prisma.user.findFirst({ where: { Identifier: UserIdentifier } });
    ...
    public async CheckSession(ctx: Context<CheckSessionByTokenDto>): Promise<{User: User, Session: Session} | null> {
        const { Token } = ctx.params;

        const session = await prisma.session.findFirst({ where: { Token } });
        if (!session) return null;
        const user = await prisma.user.findFirst({ where: { Identifier: session.UserID } });
        if (!user) return null;
